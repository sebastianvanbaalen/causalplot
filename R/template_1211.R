# Internal: template implementation for "1211"
# Layout: IV(center) -> [M1_top, M1_bottom] -> M2(center) -> DV(center)
.causal_template_1211 <- function(
    labels,
    fill_variables,
    fill_mechanisms,
    corner_radius,
  ...
  boxes <- data.frame(
    xmin  = c(1,  4,  4,  7, 10),
    xmax  = c(3,  6,  6,  9, 12),
    ymin  = c(2.4, 3.8, 1.0, 2.4, 2.4),
    ymax  = c(3.6, 5.0, 2.2, 3.6, 3.6),
    label = labels,
  ...
  # Index map:
  # 1=IV, 2=M1_top, 3=M1_bottom, 4=M2, 5=DV
  # Corrected rules:
  # - first set ends in middle of target box (left-center)
  # - second set starts from middle of source box (right-center)
  arrows <- rbind(
    ...
    data.frame( # M1_top right-center -> M2 top-left corner
      x = boxes$xmax[2], y = boxes$y[2],
      xend = boxes$xmin[4], yend = boxes$ymax[4]
    ),
    data.frame( # M1_bottom right-center -> M2 bottom-left corner
      x = boxes$xmax[3], y = boxes$y[3],
      xend = boxes$xmin[4], yend = boxes$ymin[4]
    ),
    data.frame( # M2 right-center -> DV left-center (straight)
