# Internal: template implementation for "131"
# Layout: IV(center) -> [M_top, M_mid, M_bottom] -> DV(center)
.causal_template_131 <- function(
    labels,
    fill_variables,
    fill_mechanisms,
    corner_radius,
  ...
  # Index map: 1=IV, 2=M_top, 3=M_mid, 4=M_bottom, 5=DV
  boxes <- data.frame(
    xmin  = c(1,  5,  5,  5,  9),
    xmax  = c(3,  7,  7,  7, 11),
    ymin  = c(2.4, 3.8, 2.4, 1.0, 2.4),
    ymax  = c(3.6, 5.0, 3.6, 2.2, 3.6),
    label = labels,
  ...
  # Arrows:
  # IV top-right corner    -> M_top left-center
  # IV right-center        -> M_mid left-center (straight)
  # IV bottom-right corner -> M_bottom left-center
  # M_top right-center     -> DV top-left corner
  # M_mid right-center     -> DV left-center (straight)
  # M_bottom right-center  -> DV bottom-left corner
  arrows <- rbind(
    data.frame(
