# Internal: template implementation for "1331"
# Layout: IV(center) -> [M1_top, M1_mid, M1_bottom] -> [M2_top, M2_mid, M2_bottom] -> DV(center)
.causal_template_1331 <- function(
    labels,
    fill_variables,
    fill_mechanisms,
    corner_radius,
  ...
  # Index map:
  # 1=IV,
  # 2=M1_top, 3=M1_mid, 4=M1_bottom,
  # 5=M2_top, 6=M2_mid, 7=M2_bottom,
  # 8=DV
  boxes <- data.frame(
    xmin = c(1,  4,  4,  4,  7,  7,  7, 10),
    xmax = c(3,  6,  6,  6,  9,  9,  9, 12),
    ymin = c(2.4, 3.8, 2.4, 1.0, 3.8, 2.4, 1.0, 2.4),
    ymax = c(3.6, 5.0, 3.6, 2.2, 5.0, 3.6, 2.2, 3.6),
    label = labels,
  ...
  # Arrows:
  # IV top-right corner    -> M1_top left-center
  # IV right-center        -> M1_mid left-center
  # IV bottom-right corner -> M1_bottom left-center
  # M1_top    right-center -> M2_top left-center (straight)
  # M1_mid    right-center -> M2_mid left-center (straight)
  # M1_bottom right-center -> M2_bottom left-center (straight)
  # M2_top    right-center -> DV top-left corner
  # M2_mid    right-center -> DV left-center (straight)
  # M2_bottom right-center -> DV bottom-left corner
  arrows <- rbind(
    data.frame(
    ...
    data.frame(
      ...
      xend = boxes$xmin[7], yend = boxes$y[7]
    ),
    data.frame(
      x = boxes$xmax[5], y = boxes$y[5],
      xend = boxes$xmin[8], yend = boxes$ymax[8]
    ),
    data.frame(
      x = boxes$xmax[6], y = boxes$y[6],
      xend = boxes$xmin[8], yend = boxes$y[8]
    ),
    data.frame(
      x = boxes$xmax[7], y = boxes$y[7],
      xend = boxes$xmin[8], yend = boxes$ymin[8]
    )
  )
