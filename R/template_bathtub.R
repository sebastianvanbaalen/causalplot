# Internal: template implementation for "bathtub"
# Based on "1221" but only includes the bottom process boxes.
# Layout: IV(center) -> M1_bottom -> M2_bottom -> DV(center)
# plus a direct dashed (thicker) arrow IV -> DV (straight).
.causal_template_bathtub <- function(
    labels,
    fill_variables,
    fill_mechanisms,
    corner_radius,
  ...
  boxes <- data.frame(
    xmin  = c(1,  4,  7, 10),
    xmax  = c(3,  6,  9, 12),
    ymin  = c(2.4, 1.0, 1.0, 2.4),
    ymax  = c(3.6, 2.2, 2.2, 3.6),
    label = labels,
  ...
  # Index map: 1=IV, 2=M1_bottom, 3=M2_bottom, 4=DV
  arrows_regular <- rbind(
    ...
    data.frame( # M2_bottom right-center -> DV bottom-left corner
      ...
      xend = boxes$xmin[4], yend = boxes$ymin[4]
    )
  )

  # Direct dashed arrow IV -> DV, split to avoid dash artefact under arrowhead
  head_stub <- 0.45
  arrow_direct_dashed <- data.frame(
    x = boxes$xmax[1], y = boxes$y[1],
    xend = boxes$xmin[4] - head_stub, yend = boxes$y[4]
  )
  arrow_direct_head <- data.frame(
    x = boxes$xmin[4] - head_stub, y = boxes$y[4],
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::geom_segment(
      data = arrow_direct_dashed,
      ggplot2::aes(x = x, y = y, xend = xend, yend = yend),
      linewidth = arrow_linewidth * 1.4,
      linetype = "dashed",
      lineend = "butt"
    ) +
    ggplot2::geom_segment(
      data = arrow_direct_head,
      ggplot2::aes(x = x, y = y, xend = xend, yend = yend),
      arrow = grid::arrow(type = "closed", ends = "last", length = arrow_length),
      linewidth = arrow_linewidth * 1.4,
      linetype = "solid",
